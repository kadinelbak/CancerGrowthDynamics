@testset "Future research report contract" begin
    ...
    @test occursin("Step 2A: Simplest low-resource condition", report)
    @test occursin("Step 2B: Follow-up if the shared resource effect fails", report)
    @test occursin("Step 3A: Simplest cisplatin on/off effect", report)
    @test occursin("Step 3B: Follow-up if drug action clearly lingers", report)
